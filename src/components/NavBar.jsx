...
const NavBar = () => {
  ...
  return (
    <div className='flex justify-between items-center w-full h-20 text-white fixed px-4 bg-black'>
        <div>
            <h1 className='text-5xl font-signature ml-2'>Prudhvi</h1>
        </div>

        <ul className='hidden md:flex'>
            {links.map(link => (
                <li key={link.id} className='px-4 cursor-pointer text-gray-500 hover:scale-105 duration-200 font-medium capitalize'>
        ...
        {nav && (
            ...
            {links.map(link => (
                   <li key={link.id} className='px-4 cursor-pointer text-4xl py-6 capitalize'>
